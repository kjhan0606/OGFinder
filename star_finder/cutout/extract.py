"""FITS cutout extraction and normalization for star/galaxy classification."""

import numpy as np


def extract_cutout(data, x, y, size=64):
    """Extract a cutout centered on (x, y).
...
def _resize_2d(arr, target_h, target_w):
    ...
    rf = (row_idx - r0)[:, np.newaxis]
    cf = (col_idx - c0)[np.newaxis, :]
...
def normalize_cutout(cutout, asinh_a=0.1):
    ...
    # MAD-based background subtraction
...
def batch_extract_cutouts(data, catalog, size=64):
    """Extract and normalize cutouts for all sources in catalog.

    Args:
        data: 2D FITS image array
        catalog: dict with 'x', 'y' arrays (0-based coords)
    ...
    for i in range(n):
        ...
        cut = extract_cutout(data, x, y, size=size)
        if cut is None:
            continue

        cut = normalize_cutout(cut)
        if cut is None:
            continue
